...
class YOLONCNNObjectDetectorNode(Node):
    def __init__(self):
        ...
        pkg_share = get_package_share_directory('detections')
        model_dir = os.path.join(pkg_share, 'models')
    ...
    def publish_detections(self, detections, header):
        ...
        for (x, y, w, h), score, cls in detections:
            ...
            # Create bounding box
            bbox = BoundingBox2D()
            bbox.center.position.x = float(x + w / 2.0)  # center x
            bbox.center.position.y = float(y + h / 2.0)  # center y
            bbox.center.theta = 0.0
            bbox.size_x = float(w)  # width
            bbox.size_y = float(h)  # height
            det.bbox = bbox

            # Create object hypothesis with class and confidence
            hyp = ObjectHypothesisWithPose()
            hyp.hypothesis.class_id = str(self.class_names[cls])
    ...
    def image_callback(self, msg: Image):
        try:
            ...
            self.publish_detections(detections, out_msg.header)
